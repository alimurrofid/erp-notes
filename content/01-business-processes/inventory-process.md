...
```mermaid
flowchart TD
    Phys["1. Kuantitas Fisik\n(Jumlah barang di rak/bin gudang)"]
    <--> Mov["2. Mutasi Logistik / Stock Ledger\n(Riwayat kartu stok setiap transaksi)"]
    <--> Val["3. Valuasi & Jurnal Finansial\n(Nilai saldo buku persediaan di General Ledger)"]
```
...
```mermaid
flowchart TD
    REC["1. Stock Receipt\n(Masuk: Pembelian / Retur / Produksi)"]
    --> STK["Penyimpanan di Gudang\n(Stock on Hand / Bin Location)"]
    
    STK --> TRF["2. Internal Transfer\n(Mutasi Gudang A ke Gudang B)"]
    TRF --> STK
    
    STK --> ISS["3. Stock Issue / Consumption\n(Keluar: Produksi / R&D / Sampel)"]
    
    STK --> OP["4. Stock Count / Opname\n(Pencocokan Fisik vs Sistem)"]
    OP -->|Selisih Fisik| ADJ["5. Stock Adjustment\n(Penyelarasan Nilai & Kuantitas)"]
    
    STK --> SCR["6. Scrap & Disposal\n(Pemusnahan Barang Cacat / Expired)"]
```
