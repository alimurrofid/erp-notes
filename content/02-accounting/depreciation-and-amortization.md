...
```mermaid
flowchart LR
    Cost["1. Harga Perolehan (Cost)\nTotal biaya pengadaan hingga aset siap pakai"]
    --> Minus["Dikurangi (-)"]
    --> Res["2. Nilai Residu (Residual Value)\nEstimasi nilai jual sisa di akhir masa manfaat"]
    --> Base["3. Dasar Penyusutan (Depreciable Amount)\nNilai total yang boleh disusutkan selama masa pakai"]
    --> Div["Dibagi (/)"]
    --> Life["4. Masa Manfaat (Useful Life)\nEstimasi durasi waktu pemakaian (tahun / bulan)"]
